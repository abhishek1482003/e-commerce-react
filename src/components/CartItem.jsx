...
<div className="sm:ml-24">
  <div className="form-control max-w-xs">
    ...
    <select onChange={(e)=>{dispatch(editItem({cartId,amount:e.target.value}))}} value={amount}  name="amount" id="amount" className="mt-2 select select-base select-bordered select-xs">
      {generateOptions(amount+5)};
    </select>
  </div>
<button onClick={()=>{dispatch(removeItem({cartId}))}} className="mt-2 link-primary link-hover text-sm">Remove</button>
</div>
